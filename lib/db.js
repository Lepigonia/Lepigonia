...
function isGalleryBlobUrl(url) {
  try { return /^\/gallery\//i.test(new URL(url).pathname); }
  catch { return false; }
}
...
export async function syncBlogImages() {
  ...
  for (const post of posts) {
    ...
    // A gallery image embedded in a story is a reference to the existing
    // gallery asset, not a new blog-owned gallery image. Remove any legacy
    // duplicate records that may have been created by the previous sync.
    ...
    // Only images uploaded specifically for the blog (blog/... Blob paths)
    // belong to the post's country gallery as blog-owned assets.
    const blogUrls = [...new Set(urls.filter((url) => !isGalleryBlobUrl(url)))];
