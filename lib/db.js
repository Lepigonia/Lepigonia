...
export async function syncBlogImages() {
  ...
  // Clean up records produced by older versions. A gallery-owned image is
  // never a blog-owned gallery record, even when it appears in a story.
  // NULL post_slug records are also stale and must not survive forever.
  if (active.length) {
