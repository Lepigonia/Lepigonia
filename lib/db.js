...
export async function syncBlogImages() {
  ...
  for (const post of posts) {
    ...
    // Images selected from an existing gallery are references only. They must
    // never be copied into the post's country gallery and must never become
    // owned by the post lifecycle.
    ...
    // Only images uploaded into /blog/ belong to the story. These records are
    // the only gallery records that may be removed when the story is deleted.
  ...
  // Reconcile records from older versions. Gallery-owned assets live under
  // /gallery/ and must remain manual. Story-owned assets live under /blog/
  // and are rebuilt from the currently existing posts on every gallery load.
  // A legacy manual record pointing to /blog/ is not a real gallery asset.
