...
async function reconcileManualGalleryOwnership(db) {
  ...
  // A blog-owned Blob is never a manual gallery asset. Keep the Blob itself
  // untouched: its owning story controls its lifecycle.
...
export async function syncBlogImages() {
  ...
  for (const post of posts) {
    ...
    // Existing gallery images are references, never post-owned assets.
    // A Norway gallery image used inside a Germany story stays a Norway image.
    ...
    // Only /blog/ Blob assets are owned by a story and participate in the
    // story lifecycle. Orphaned records are deliberately retained so an admin
    // can see and explicitly delete the underlying Blob later.
  ...
  // IMPORTANT: Do not delete stale blog rows here. They represent orphaned
  // Blob assets from stories that no longer exist. Keeping them makes them
  // visible to the admin as "verwaist" and gives the admin an explicit,
  // auditable way to remove the underlying Blob. They are never reattached to
  // a new post merely because the slug happens to be reused.
}
