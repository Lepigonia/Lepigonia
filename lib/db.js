...
// Restore any legacy countries that are missing, but never overwrite a renamed DB country.
// This is intentionally idempotent and is safe to run on every gallery read.
export async function seedGalleryFromLegacy(){
 ...
 try{
  ...
  for(const c of legacy.countries||[]){
   ...
   const existing=await db`SELECT id FROM gallery_countries WHERE id=${id} OR slug=${slug} LIMIT 1`;
   if(!existing.length){
    await db`INSERT INTO gallery_countries(id,slug,name) VALUES(${id},${slug},${name}) ON CONFLICT DO NOTHING`;
   }
   const country=(await db`SELECT id FROM gallery_countries WHERE id=${id} OR slug=${slug} LIMIT 1`)[0];
   if(!country)continue;
   for(const i of c.images||[]){
    ...
    await db`INSERT INTO gallery_images(id,country_id,source,post_slug,url,filename,storage,title,created_at)
      VALUES(${i.id||`legacy-${Date.now()}-${Math.random()}`},${country.id},${i.source==="blog"?"blog":"manual"},${i.postSlug||null},${i.url},${i.filename||null},${i.storage||"blob"},${i.title||null},${i.createdAt||new Date().toISOString()})
...
async function findCountry(db,name){
 ...
 if(c)return c;
 await db`INSERT INTO gallery_countries(id,slug,name) VALUES(${s},${s},${n}) ON CONFLICT(slug) DO NOTHING`;
...
export async function syncBlogImages(){
 ...
 // Remove only stale blog-image records. Never delete countries here: an empty country is a valid
 // admin-created gallery and must remain visible until the administrator explicitly deletes it.
 if(active.length)await db`DELETE FROM gallery_images WHERE source='blog' AND NOT(post_slug=ANY(${active}))`;
...
export async function getGallery({syncPosts=true}={}){
 ...
 for(const i of images){if(!map.has(i.country_id))map.set(i.country_id,[]);map.get(i.country_id).push({id:i.id,url:i.url,source:i.source,postSlug:i.post_slug,filename:i.filename,storage:i.storage,title:i.title,createdAt:i.created_at})}
