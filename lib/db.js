...
async function reconcileManualGalleryOwnership(db) {
  ...
  // A manual gallery upload contains its owning country in its canonical Blob
  // path: /gallery/<country-slug>/<filename>. If an old record was attached
  // to the wrong country, move the database row to the country encoded in the
  // Blob path. The Blob itself is untouched, so it remains available in the
  // correct gallery.
  ...
  // Older versions could accidentally register a blog-owned Blob as a manual
  // gallery image. Such a record is never a valid gallery asset. The actual
  // Blob is deliberately not deleted here because an existing post may still
  // reference it; the post lifecycle owns Blob deletion.
...
export async function syncBlogImages() {
  ...
  for (const post of posts) {
    ...
    // Existing gallery images are references, never post-owned assets. They
    // must never be inserted into gallery_images for the post's country.
    ...
    // Only /blog/ Blob assets are owned by a story and therefore participate
    // in the story deletion lifecycle.
  ...
  // Remove stale blog records after rebuilding the set from the currently
  // existing posts. This also cleans up records left by deleted stories.
  if (active.length) {
    await db`DELETE FROM gallery_images
      WHERE source='blog'
        AND (post_slug IS NULL OR NOT(post_slug=ANY(${active})) OR url LIKE '%/gallery/%')`;
  } else {
