...
export async function getGallery({ syncPosts = true } = {}) {
  ...
  for (const image of images) {
    ...
    byCountry.get(image.country_id).push({ id:image.id,url:image.url,source:image.source,postSlug:image.post_slug,filename:image.filename,storage:image.storage,title:image.title,createdAt:image.created_at });
  }
...
export async function syncBlogImages() {
  ...
  for (const post of posts) {
    ...
    // Preserve an existing post-to-country relation. This is important when an
    // admin renames a gallery country (e.g. Norway -> Norwegen): the post may
    // still contain its historical country label, but its gallery images must
    // remain attached to the renamed database country.
    const existing = (await db`SELECT country_id FROM gallery_images WHERE source='blog' AND post_slug=${post.slug} LIMIT 1`)[0];
    ...
    await db`INSERT INTO gallery_images (id,country_id,source,post_slug,url,filename,storage,title)
      VALUES (${`post-${post.slug}`},${countryId},'blog',${post.slug},${post.image},NULL,'blob',${post.title || "Blogartikel"})
