...
const COUNTRY_FLAGS = {
  ...
  uk: "🇬🇧", "united kingdom": "🇬🇧", england: "🇬🇧", ireland: "🇮🇪", morocco: "🇲🇦", marokko: "🇲🇦",
...
function sortPosts(posts) {
  return posts.map((post, index) => ({ ...post, _sourceIndex: index }))
...
export default function WorldMap({ posts = [] }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const stopLabel = language === "DE" ? "Reisestopp" : "Travel stop";
    const storyLabel = language === "DE" ? "Story öffnen" : "Open story";
    const mapAria = language === "DE" ? "Interaktive Weltkarte der Reisestopps" : "Interactive world map of travel stops";
    const invalidate = () => { if (map) window.requestAnimationFrame(() => map?.invalidateSize({ pan: false, animate: false })); };
    ...
    const init = async () => {
      ...
      if (!valid.length) map.setView([20, 0], 2);
        ...
        valid.forEach((post, index) => {
          ...
          const location = post.location || post.country || stopLabel;
          const title = post.title || stopLabel;
          const icon = L.divIcon({ className: "lepigonia-map-marker-wrap", html: `<span class="lepigonia-map-marker"><span class="lepigonia-map-flag">${flag}</span><b>${index + 1}</b></span>`, iconSize: [58, 40], iconAnchor: [29, 20], popupAnchor: [0, -18] });
          const marker = L.marker([lat, lng], { icon }).addTo(map);
          marker.bindTooltip(`<strong>${index + 1} · ${escapeHtml(location)}</strong><br>${escapeHtml(title)}`, { direction: "top", offset: [0, -18], opacity: 0.98 });
          marker.bindPopup(`<article class="map-popup"><span class="map-popup-kicker">${flag} ${escapeHtml(location)}</span><strong>${escapeHtml(title)}</strong>${post.date ? `<small>${escapeHtml(post.date)}</small>` : ""}<a href="/blog/${encodeURIComponent(post.slug)}">${storyLabel} <span>→</span></a></article>`);
        });
  ...
  return <div className="world-map-shell"><div ref={ref} className="world-map" aria-label={language === "DE" ? "Interaktive Weltkarte der Reisestopps" : "Interactive world map of travel stops"} /><div className="map-legend"><span className="map-legend-dot" /> {language === "DE" ? "Reisestopps" : "Travel stops"} <span className="map-legend-line" /> {language === "DE" ? "Reiseroute" : "Route"}</div><p className="map-credit">{language === "DE" ? "Kartendaten" : "Map data"} &copy; OpenStreetMap contributors</p></div>;
}
