...
export async function GET(request) {
  ...
  return NextResponse.json((await readData()).data);
}
...
export async function POST(request) {
  ...
  try {
    ...
    if (body.action === "country-update") {
      ...
      c.name = name; await writeData(data, sha); return NextResponse.json({ ok: true });
    }
    ...
    if (body.action === "image-add") {
      ...
      const bytes = Math.floor(match[2].length * 3 / 4); if (bytes > 1.05 * 1024 * 1024) return NextResponse.json({ error: "Das optimierte Bild ist noch zu groß." }, { status: 400 });
      const filename = `${Date.now()}-${Math.random().toString(36).slice(2,8)}.webp`; await putGithubBinary(`public/uploads/gallery-${filename}`, prepared, `Add gallery image: ${filename}`);
