...
export default function BlogBlockEditor({ value, onChange, country }) {
  ...
  async function openImages(){
    setShowImages(true);setShowImageOptions(null);setError("");setGalleryLoading(true);
    try{
      ...
      // Always load the complete gallery. The current country is shown as a label,
      // but it must never hide images from other countries.
  ...
  return <div className="block-editor">
    ...
    {showImages&&<div className="block-modal" role="dialog" aria-modal="true"><div className="block-modal-card"><button type="button" className="block-modal-close" onClick={()=>setShowImages(false)}>×</button><p className="admin-eyebrow">Bild einfügen</p><h3>{country?`Alle Fotos · aktuelles Land: ${country}`:"Alle Galeriefotos"}</h3><label className="block-upload">＋ Neues Bild hochladen<input type="file" accept="image/*" onChange={uploadInline}/></label>{galleryLoading?<p>Galerie wird geladen …</p>:<div className="block-image-grid">{gallery.map(image=><button type="button" key={`${image.id||image.url}-${image.country||""}`} className="block-image-choice" title={image.country||"Galerie"} onClick={()=>chooseImage(image)}><img src={image.url} alt={image.title||`${image.country||"Galerie"} Foto`}/><span className="block-image-country">{image.country}</span></button>)}</div>}{!galleryLoading&&!gallery.length&&<p className="block-muted">Noch keine Galeriebilder vorhanden.</p>}</div></div>}
